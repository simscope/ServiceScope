...
await db.query('insert into auth.users(id) select unnest($1::uuid[])', [[
  ids.user, ids.otherUser, ids.manager, ids.dispatcher, ids.technician, ids.technicianFull, ids.explicitOff, ids.disabled,
]]);
...
await db.query(`insert into public.company_users(id,company_id,auth_user_id,name,email,role,status,portal_access_rules) values
  ($1,$8,$1,'Manager','manager@example.com','manager','active','{"aiAssistant":"full"}'),
  ($2,$8,$2,'Dispatcher','dispatcher@example.com','dispatcher','active','{}'),
  ($3,$8,$3,'Technician','technician@example.com','technician','active','{}'),
  ($4,$8,$4,'Technician Full','technician-full@example.com','technician','active','{"aiAssistant":"full"}'),
  ($5,$8,$5,'Explicit Off','off@example.com','manager','active','{"aiAssistant":"off"}'),
  ($6,$8,$6,'Disabled','disabled@example.com','manager','disabled','{"aiAssistant":"full"}'),
  ($7,$9,$7,'Other Manager','other@example.com','manager','active','{"aiAssistant":"full"}')`,
  [ids.manager, ids.dispatcher, ids.technician, ids.technicianFull, ids.explicitOff, ids.disabled, ids.otherUser, ids.company, ids.otherCompany]);
...
const claim1 = await db.query('select * from public.claim_company_reel_render_job($1)', [first.render_job_id]);
...
for (const key of ['plans_rls','jobs_rls','auth_read','service_claim','service_release','service_ai_helper']) check(() => assert.equal(tableSecurity.rows[0][key], true));
for (const key of ['auth_plan_select','auth_job_update','auth_claim','auth_release','auth_ai_helper','anon_read']) check(() => assert.equal(tableSecurity.rows[0][key], false));
