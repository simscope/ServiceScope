...
check(() => assert.match(errors, /REEL_RENDER_CONTEXT_STALE/));
check(() => assert.match(authorization, /validAuthorityContext\(context\)[\s\S]*REEL_RENDER_CONTEXT_STALE/));
check(() => assert.match(authorization, /invalidPlanErrorCodes\.has\(error\?\.message\)[\s\S]*REEL_RENDER_INVALID_PLAN/));
