...
const [
  ...
  read('server/reel-render-jobs/approval.js'),
  ...
  read('server/reel-render-jobs/telemetry.js'),
  read('api/reel-plan-approve.js'),
...
check(() => assert.match(approval, /approve_company_reel_creative_plan/));
check(() => assert.match(approvalApi, /createReelApprovalHandler/));
check(() => assert.match(client, /'\/api\/reel-plan-approve'/));
...
check(() => assert.match(repository, /p_video_sha256: metadata\.videoSha256/));
check(() => assert.match(repository, /p_cover_sha256: metadata\.coverSha256/));
check(() => assert.match(worker, /createHash\('sha256'\)/));
check(() => assert.match(worker, /coverFileSize: coverBytes\.byteLength/));
...
check(() => assert.match(worker, /render_blocked_privacy/));
...
const begin = controlledMigration.match(/create or replace function public\.begin_company_reel_render_request[\s\S]*?\$\$;/)?.[0] ?? '';
...
check(() => assert.match(begin, /can_manage_company_ai_assistant\(plan\.company_id\)/));
...
check(() => assert.match(begin, /company_reel_creative_plan_approvals/));
check(() => assert.match(begin, /REEL_RENDER_APPROVAL_REQUIRED/));
check(() => assert.match(controlledMigration, /revoke all on function public\.begin_company_reel_render_request\(uuid,text\) from public, anon/));
check(() => assert.match(controlledMigration, /grant execute on function public\.begin_company_reel_render_request\(uuid,text\) to authenticated/));
check(() => assert.match(controlledMigration, /company_reel_render_jobs_artifact_integrity_check/));
check(() => assert.match(controlledMigration, /company_reel_creative_plan_approvals_immutable/));
check(() => assert.match(controlledMigration, /company_reel_render_jobs_transition_guard/));
check(() => assert.match(controlledMigration, /old\.status = 'queued' and new\.status = 'rendering'/));
check(() => assert.match(controlledMigration, /old\.status = 'rendering' and new\.status in \('rendering', 'completed', 'failed'\)/));
check(() => assert.match(controlledMigration, /revoke all on function public\.protect_company_reel_render_job_transition\(\) from public, anon, authenticated/));
check(() => assert.match(controlledMigration, /'REEL_PRIVACY_FAILED'/));
check(() => assert.match(contracts, /if \(message\.includes\('REEL_PRIVACY_FAILED'\)\) return 'REEL_PRIVACY_FAILED'/));
...
const aiManage = controlledMigration.match(/create or replace function public\.can_manage_company_ai_assistant[\s\S]*?\$\$;/)?.[0] ?? '';
check(() => assert.match(aiManage, /company\.owner_email/));
check(() => assert.match(aiManage, /company_user\.status = 'active'/));
check(() => assert.match(aiManage, /profile\.access_rules->>'aiAssistant'[\s\S]*end = 'full'/));
check(() => assert.match(aiManage, /company_user\.portal_access_rules->>'aiAssistant'[\s\S]*end = 'full'/));
check(() => assert.match(controlledMigration, /revoke all on function public\.can_manage_company_ai_assistant\(uuid\) from public, anon, authenticated/));
check(() => assert.match(controlledMigration, /grant execute on function public\.can_manage_company_ai_assistant\(uuid\) to service_role/));
const approvalRpc = controlledMigration.match(/create or replace function public\.approve_company_reel_creative_plan[\s\S]*?\$\$;/)?.[0] ?? '';
check(() => assert.match(approvalRpc, /can_manage_company_ai_assistant\(plan\.company_id\)/));
check(() => assert.match(approvalRpc, /existing\.plan_revision = plan\.plan_revision/));
