...
check(() => assert.match(edgeSource, /requireVerifiedAuthUserId\(authResult\)/));
check(() => assert.match(edgeSource, /assertMetaAccessRole\(session, companyId\)/));
check(() => assert.match(serverSource, /kind === 'owner' && role === 'owner'/));
check(() => assert.match(serverSource, /role === 'admin' \|\| role === 'manager'/));
