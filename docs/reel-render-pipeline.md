...
An authenticated user first approves the exact persisted plan revision through `approve_company_reel_creative_plan`. `begin_company_reel_render_request` fails closed unless that immutable approval exists. The render fingerprint binds the plan revision and JSON snapshot to the renderer, presentation, and output-contract versions. Render completion never invokes a social provider and does not grant publication approval.
...
Claims are atomic and lease based. A concurrent worker receives no claim, completed and failed jobs are terminal, and an expired lease can be reclaimed up to five total attempts. Identical immutable requests resolve to the same render job and deterministic object keys. Transient Sandbox, queue, and storage failures are retried within those bounds; privacy, ownership, stale context, unsupported media, invalid plans, and invalid output are terminal.
