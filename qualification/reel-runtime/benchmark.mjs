...
async function runFixture(name, outputDir) {
  ...
  try {
    ...
    const ffmpeg = JSON.parse(await readFile(timingFile, 'utf8'));
    ...
    const report = {
      ...
      performance: {
        ...
        ffmpegWallMs: round(ffmpeg.wallSeconds * 1000),
        ffmpegUserCpuMs: round(ffmpeg.userSeconds * 1000),
        ffmpegSystemCpuMs: round(ffmpeg.systemSeconds * 1000),
        ffmpegMaxRssBytes: ffmpeg.maxRssKb * 1024,
...
async function createWorstNormalFixture(root) {
  ...
  return {
    ...
    expectedScenes: 7,
    frameTimes: [['hook', '0.20'], ['transition', '6.45'], ['middle', '12.20'], ['brand-final', '24.20']],
  };
}
...
function resourceSamplesCsv(samples) {
  const fields = ['elapsedMs', 'memoryCurrentBytes', 'cpuUsageUsec', 'cpuPercent', 'tmpBytes', 'rendererWorkBytes', 'normalizedSourceBytes'];
...
async function combineReports(root) {
  ...
  const report = {
    schemaVersion: 'servicescope-reel-runtime-qualification-v1',
    baselineSha: '792b74b71369eec49a9975d38f2cbecb12b8731c',
    runner,
    container: {
      baseImage,
...
| Fixture | Status | Duration ms | Renderer wall ms | FFmpeg wall ms | Peak memory bytes | Peak /tmp bytes | MP4 bytes | Cover bytes |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
