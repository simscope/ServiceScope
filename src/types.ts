...
export type CompanyPortalAccessPage = 'jobInbox' | 'jobs' | 'allJobs' | 'debtors' | 'calendar' | 'materials' | 'warehouse' | 'tasks' | 'map' | 'email' | 'finances' | 'aiBusiness' | 'aiAssistant' | 'knowledge' | 'import' | 'portal' | 'onboarding';
...
export type Company = {
  ...
  alerts: string[];
  accessRules?: CompanyAccessRules;
  usage: {
...
export type NewCompanyForm = Pick<
  Company,
  'name' | 'ownerName' | 'phone' | 'ownerEmail' | 'temporaryPassword' | 'domain' | 'market' | 'plan' | 'status'
>;
