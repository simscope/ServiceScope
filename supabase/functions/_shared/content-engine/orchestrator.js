...
export async function generateWithProvider({ request, context, provider, config, telemetry, clock = Date }) {
  ...
  try {
    ...
    emitTelemetry(telemetry, { correlationId: request.idempotencyKey, provider: provider?.id ?? 'deterministic-fallback', model: config.model, channel: request.channel, promptVersion: request.promptVersion, success: false, code, latencyMs: clock.now() - start, attempts: config.maxAttempts });
...
async function callWithRetry(provider, providerRequest, config) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      lastError = controller.signal.aborted ? new Error('PROVIDER_TIMEOUT') : error;
...
function isRetryable(error) {
  const message = error instanceof Error ? error.message : String(error);
  return message.includes('PROVIDER_TIMEOUT') || message.includes('PROVIDER_UNAVAILABLE');
}
