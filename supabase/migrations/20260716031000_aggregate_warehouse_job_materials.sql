-- Aggregate warehouse-backed Job material rows by Job + inventory item.
-- Multiple stock issues of the same item to the same Job should increase quantity
-- on one material line, while inventory movements keep the stock audit trail.
...
ranked as (
  select
    *,
    first_value(id) over (partition by company_id, job_id, item_id order by created_at, id) as keep_id,
    row_number() over (partition by company_id, job_id, item_id order by created_at, id) as row_number,
    sum(quantity) over (partition by company_id, job_id, item_id) as total_quantity,
    case
      when sum(quantity) over (partition by company_id, job_id, item_id) = 0 then max(unit_price_cents) over (partition by company_id, job_id, item_id)
      else round(sum(quantity * unit_price_cents) over (partition by company_id, job_id, item_id) / sum(quantity) over (partition by company_id, job_id, item_id))::integer
    end as weighted_unit_price_cents
  from warehouse_rows
),
updated as (
  update public.job_materials jm
    set quantity = ranked.total_quantity,
        unit_price_cents = ranked.weighted_unit_price_cents,
        status = case when ranked.total_quantity <= 0 then 'Returned'::public.material_status else 'Installed'::public.material_status end,
        updated_at = now()
  from ranked
  where jm.id = ranked.keep_id
    and ranked.row_number = 1
    and exists (
      select 1
      from ranked duplicate_check
      where duplicate_check.keep_id = ranked.keep_id
        and duplicate_check.row_number > 1
    )
  returning jm.id
)
delete from public.job_materials jm
using ranked
where jm.id = ranked.id
  and ranked.row_number > 1;
...
declare
  ...
  v_existing_job_material public.job_materials%rowtype;
  ...
  v_next_material_quantity numeric(14,4);
  v_movement_id uuid;
...
begin
  ...
  select jm.* into v_existing_job_material
  from public.job_materials jm
  join public.inventory_movements im on im.id = jm.inventory_movement_id
  where jm.company_id = v_job.company_id
    and jm.job_id = p_job_id
    and jm.source_type = 'warehouse'
    and im.movement_type = 'job_issue'
    and im.item_id = p_item_id
  ...
  if v_existing_job_material.id is not null then
    v_next_material_quantity := v_existing_job_material.quantity + v_quantity;

    update public.job_materials
      set quantity = v_next_material_quantity,
          unit_price_cents = case
            when v_next_material_quantity <= 0 then v_existing_job_material.unit_price_cents
            else round(((v_existing_job_material.quantity * v_existing_job_material.unit_price_cents) + (v_quantity * v_unit_cost * 100)) / v_next_material_quantity)::integer
          end,
          status = 'Installed',
          updated_at = now()
    where id = v_existing_job_material.id
