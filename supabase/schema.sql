...
create table public.company_reel_render_jobs (
  ...
  constraint company_reel_render_jobs_artifact_integrity_check check (
    (status = 'completed'
...
begin
  ...
  if approval.id is null then
    select existing.* into approval from public.company_reel_creative_plan_approvals existing
    where existing.creative_plan_id = plan.id and existing.company_id = plan.company_id
      and existing.job_id = plan.job_id and existing.plan_revision = plan.plan_revision;
  end if;
